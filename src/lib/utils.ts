...
export const getHighlightRange = (highlight: number | number[] | null | undefined) => {
    /**
     * Set to Range(0, 0) if:
    ...
    if (
        !highlight ||
        (Array.isArray(highlight) && (isNaN(highlight[0]) || isNaN(highlight[1]))) ||
...
export const bufferConcat = (a: Uint8Array, b: Uint8Array): Uint8Array => {
...
export const convertBufferToLines = (current: Uint8Array, previous?: Uint8Array | null) => {
...
export interface SearchFormatPartOptions {
    searchKeywords: string;
    nextFormatPart?: ((part: string) => string) | undefined;
    caseInsensitive: boolean;
    replaceJsx: (text: string, key: number) => React.ReactNode;
    selectedLine?: boolean;
    replaceJsxHighlight?: (text: string, key: number) => React.ReactNode;
    highlightedWordLocation?: number;
}
...
export const searchFormatPart =
    ...
    }: SearchFormatPartOptions) =>
    (part: string) => {
        let formattedPart: string | React.ReactNode[] = part;
